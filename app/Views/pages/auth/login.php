...
<form id="apx-login-form" action="<?= route('/login') ?>" method="post" novalidate>
  ...
  <button class="apx-btn apx-btn--primary apx-btn--block apx-mt-3" type="submit"><span class="apx-btn-label"><?= __('auth.login.submit') ?></span></button>
</form>

<div class="apx-auth__alt">
  <a href="<?= route('/forgot-password') ?>"><?= __('auth.forgot.title') ?></a> · <?= __('auth.no_account') ?>
...
<script type="module">
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      errBox.textContent = (err.errors && Object.values(err.errors)[0]) || err.message;
  ...
  function renderTotp() {
    ...
    form.innerHTML = ''
      ...
      +   '<span class="apx-field__bar"></span>'
      + '</div>'
      + '<button class="apx-btn apx-btn--primary apx-btn--block apx-mt-3" type="submit"><span class="apx-btn-label">' + L.confirm + '</span></button>';
